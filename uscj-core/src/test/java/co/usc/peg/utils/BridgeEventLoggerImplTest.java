...
public class BridgeEventLoggerImplTest {
    ...
    public void logCommitFederation() {
        ...
        for(int i = 0; i < 4; i++) {
            Assert.assertEquals(oldFederation.getPublicKeys().get(i), UldECKey.fromPublicOnly(oldFedPubKeys.get(i).getRLPData()));
        }
        ...
        for(int i = 0; i < 3; i++) {
            Assert.assertEquals(newFederation.getPublicKeys().get(i), UldECKey.fromPublicOnly(newFedPubKeys.get(i).getRLPData()));
        }
